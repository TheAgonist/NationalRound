...
function MIDIUtils.create(data_dir, batch_size, seq_length, split_fractions)
  ...
  --self.x_batches = data:view(127, -1):view(batch_size,-1):split(seq_length, 2)  -- #rows = #batches
  --print(data:view(batch_size,-1))
  ...
  --self.y_batches = ydata:view(127, -1):view(batch_size,-1):split(seq_length, 2)  -- #rows = #batches
...
function MIDIUtils.text_to_tensor(in_midifile, out_vocabfile, out_tensorfile)
  ...
  --print(tot_len)
  local data = torch.ByteTensor(tot_len,127) -- store it into 1D first, then rearrange
  data= data:fill(1)
  local currlen = 0
  for i=0,#datas do
    ...
    for i=1, #rawdata[indexer] do
      if rawdata[indexer][i] ~= nil then
        if rawdata[indexer][i][1]=="note" and rawdata[indexer][i][2]~=0 then
            ...
            data[(currlen+(rawdata[indexer][i][2] )/(rawdata[1]/4)+1)][rawdata[indexer][i][5]] = vocab_mapping[rawdata[indexer][i][5]+ 127*tempIndex]
            --print(data[currlen+currnote].."   "..vocab_mapping[rawdata[indexer][i][5]+ 127*tempIndex])
           if data[(currlen+(rawdata[indexer][i][2] )/(rawdata[1]/4)+1)][rawdata[indexer][i][5]]==0 then data[(currlen+(rawdata[indexer][i][2] )/(rawdata[1]/4)+1)][rawdata[indexer][i][5]]=data[currlen+currnote]+1
           --print(data[currlen+currnote].."   "..vocab_mapping[rawdata[indexer][i][5]+ 127*tempIndex]) 
           --os.exit()
         end
        end 
